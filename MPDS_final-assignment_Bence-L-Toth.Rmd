...
There are numerous studies done on the health effects of alcohol and how it can change the life span of people. Just by typing *"alcohol life expectancy study"* into Google, we end up with around 11,000,000 results. Some may clain that [moderate drinking can actually be beneficial to our health](https://www.acc.org/latest-in-cardiology/articles/2017/08/14/12/54/consuming-alcohol-in-moderation-can-lower-mortality-risks), while most studies suggest that [Even one drink a day linked to lower life expectancy](https://www.reuters.com/article/us-health-alcohol/even-one-drink-a-day-linked-to-lower-life-expectancy-idUSKBN1I42H6).

This analysis examines the pattern of association between alcohol consumption and average life expectancy of various countries, explaining the possible correlations and offer an insight on why these correlations might be present.
...
We need a way to aggregate the servings in a meaningful way: french people might consume more wine, while germans might drink more beer, accoring to well-known stereotypes. Nevertheless, we need a way to compare these countries on a similar scale.
...
I focues on life expectancy at birth for both sexes in 2012 and joined the `drinks` data by country.
...
The `countries` data is a bit messier than the other two. It is stored as an *.xlsx* file. We skipped the first 3 non-imporant lines on sheet 1 during import. The (now) first line contains the headers, and some part of it is left in the second line as well.
...
The hypotheses seems to be right indeed. More developed regions such as *Europe*, the *Americas* and the *Western Pacific* (Australia, New Zealand etc.) seem to have a positive relationship according to the `loess` regression. On the other hand, less developed regions like *Africa* and *Eastern Mediterranean* tend to consume less alcohol due to economical/cultural/religious reasons, and their life expectancy is not affected by the magnitude of drink intake.
